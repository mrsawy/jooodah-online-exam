...
export const setLevelThunk = createAsyncThunk("level/setLevel", async (args, thunkAPI) => {
  try {
    ...
    let levels = args.map((level) => ({ ...level, questions: [] }));

    const level = await setLevelService(levels);
    return level;
  } catch (error) {
    return thunkAPI.rejectWithValue(error);
  }
...
export const levelSlice = createSlice({
  ...
  reducers: {
    ...
    editLevelItem: (state, action) => {
      const { _id, numberOfMinutes, name_en, name_ar, pauseTime, level_ar, level_en } =
        action.payload;
      state.levels = state.levels.map((ele) =>
        ele._id === _id
          ? { ...ele, name_en, name_ar, numberOfMinutes, pauseTime, level_ar, level_en }
          : ele
      );
    },
